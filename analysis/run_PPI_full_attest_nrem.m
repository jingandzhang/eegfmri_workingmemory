...
for iss=28:29%1:length(ss_list)
    ss = ss_list{iss};
...
folder_name = 'spm_nrem_model1';
voi_list= {'R_Hipp','L_Hipp','R_Thal','L_Thal','vmPFC'}%{'Post_R_Hipp','Post_L_Hipp','Mid_R_Hipp','Mid_L_Hipp','Ant_R_Hipp','Ant_L_Hipp'};
%{'R_Hipp','L_Hipp','R_Thal','L_Thal','vmPFC',{'Post_R_Hipp','Post_L_Hipp','Mid_R_Hipp','Mid_L_Hipp','Ant_R_Hipp','Ant_L_Hipp'};%,'L_Caud','R_Caud','L_Puta','R_Puta','L_Stri','R_Stri','R_Hipp','L_Hipp','R_Accu','L_Accu','R_Pall','L_Pall','R_Lc','L_Lc','R_Thal','L_Thal'};
Ntrs = {};
Nscans = [];


for iss=27:29%1:length(ss_list)
    ss = ss_list{iss};
    ss_dir = [preproc_dir ss];
    sessions = glob('ses*', ss_dir);
    if isempty(sessions)
    ...
    %loop for sessions
    for i_ses= 3%1:2:length(sessions)
        %%
        [filepath,name] = fileparts(char(sessions(i_ses)));
        ses = name; %session is equal to the name of the folder 
