...
so_dir = '/mnt/tambinidata/sleepstudy/data/derivatives/group/sp_nrem/';
so_ss_fls = glob('v*.nii', so_dir);
% so_ss_fls = glob('v*.nii.gz', so_dir);
% for i=1:length(so_ss_fls)
%     unix(['gunzip ' so_ss_fls{i}])
% end
for i=1:length(so_ss_fls)
...
for ii=1:size(reshaped_os_data,2)
    ...
    [coeff,stats,r] = arobustfit(reshaped_os_data(:,ii),reshaped_so_data(:,ii));
    slope_idx = 2;
...
new_info_os_r.fname= '/mnt/tambinidata/sleepstudy/data/derivatives/group/spindle_vs_ampm_corr/voxelwise_sp_ampm_corr_rmap.nii';
spm_write_vol(new_info_os_r, reshaped_3d_r);
new_info_os_p=info_os;
new_info_os_p.fname= '/mnt/tambinidata/sleepstudy/data/derivatives/group/spindle_vs_ampm_corr/voxelwise_sp_ampm_corr_pmap.nii';
...
fdr_pos_voxel_r = mafdr(pos_reshaped_3d_r(:),'BHFDR',true);
fdr_pos_voxel_p = mafdr(pos_reshaped_3d_p(:),'BHFDR',true);
fdr_neg_voxel_r = mafdr(neg_reshaped_3d_r(:),'BHFDR',true);
...
fdr_pos_new_info_os_r.fname= '/mnt/tambinidata/sleepstudy/data/derivatives/group/spindle_vs_ampm_corr/fdr_pos_voxelwise_sp_ampm_corr_rmap_wholebrain.nii';
spm_write_vol(fdr_pos_new_info_os_r,fdr_pos_voxel_r);
fdr_pos_new_info_os_p=info_os;
fdr_pos_new_info_os_p.fname= '/mnt/tambinidata/sleepstudy/data/derivatives/group/spindle_vs_ampm_corr/fdr_pos_voxelwise_sp_ampm_corr_pmap_wholebrain.nii';
spm_write_vol(fdr_pos_new_info_os_p, 1-fdr_pos_voxel_p);

fdr_neg_new_info_os_r=info_os;
fdr_neg_new_info_os_r.fname= '/mnt/tambinidata/sleepstudy/data/derivatives/group/spindle_vs_ampm_corr/fdr_neg_voxelwise_sp_ampm_corr_rmap_wholebrain.nii';
spm_write_vol(fdr_neg_new_info_os_r,fdr_neg_voxel_r);
fdr_neg_new_info_os_p=info_os;
fdr_neg_new_info_os_p.fname= '/mnt/tambinidata/sleepstudy/data/derivatives/group/spindle_vs_ampm_corr/fdr_neg_voxelwise_sp_ampm_corr_pmap_wholebrain.nii';
