...
folder_name = 'spm_fsttest_model1';
...
spm_fls = {'spmT';'con'};
onset_dir=[analysis_dir '/fst_event_files'];

%for loop for subjects 
for iss=18%1:length(ss_list)
    ...
    for i_ses= 1:length(sessions)
        ...
        nuis_file = glob('*fsttest_nuisance_test.txt', func_dir, 0); % (using 0 as 3rd input to glob will output string)
        ...
        onset_times_files = glob('*Test*Data.mat',onset_dir,0);
        a=spm_dir(62:76);
        a=regexp(a,'[0-9]','match');
        a=cell2mat(a);
        if str2num(a(end))==1
            temp_names=onset_times_files(find(contains(onset_times_files,'Visit1')));
            temp_names=temp_names(find(contains(temp_names,'Test1')));
            temp_names=temp_names(find(contains(temp_names,a(1:end-1))));
        elseif str2num(a(end))==2
            temp_names=onset_times_files(find(contains(onset_times_files,'Visit1')));
            temp_names=temp_names(find(contains(temp_names,'Test2')));
            temp_names=temp_names(find(contains(temp_names,a(1:end-1))));
        elseif str2num(a(end))==3
            temp_names=onset_times_files(find(contains(onset_times_files,'Visit2')));
            temp_names=temp_names(find(contains(temp_names,'Test1')));
            temp_names=temp_names(find(contains(temp_names,a(1:end-1))));    
        elseif str2num(a(end))==4
            temp_names=onset_times_files(find(contains(onset_times_files,'Visit2')));
            temp_names=temp_names(find(contains(temp_names,'Test2')));
            temp_names=temp_names(find(contains(temp_names,a(1:end-1))));          
        end
        onset_times=char(temp_names);
        ...
        for iscan = 1:Nscans
            ...
            % session
            scan_fl = glob([ss '_' ses '_' 'task-fsttest_run-0' num2str(iscan) '_*' '_S1.nii'],...
                func_dir, 0);
        ...
        matlabbatch{1}.spm.stats.fmri_spec.mthresh = -Inf; % modify if you want to use spm's automatic mask - try 0.5
         fls_zip = glob(['*combined-mask.nii'], ss_dir);
